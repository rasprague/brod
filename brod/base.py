...
class BaseKafka(object):
    ...
    def produce(self, topic, messages, partition=None, callback=None, compression=COMPRESSION_NONE):
        ...
        # Encode the request
        request = self._produce_request(topic, messages, partition, compression)
    ...
    def _produce_request(self, topic, messages, partition, compression=COMPRESSION_NONE):
        ...
        if self.version_0_7:
            if compression == COMPRESSION_NONE:
                ...
                compressed_message_set = gzip_compress(message_set_buffer.getvalue())
                encoded_compressed_message_set = struct.pack('>BBi{0}s'.format(len(compressed_message_set)),
                    MAGIC_BYTE_07,
                    COMPRESSION_GZIP,
                    self.compute_checksum(compressed_message_set),
                    compressed_message_set
                )
                compressed_message_set_buffer = StringIO()
                compressed_message_size = len(encoded_compressed_message_set)
                compressed_bin_format = '>i{0}s'.format(compressed_message_size)
                compressed_message_set_buffer.write(struct.pack(compressed_bin_format, compressed_message_size,
                    encoded_compressed_message_set))
                message_set = compressed_message_set_buffer.getvalue()
...
class Partition(object):
    ...
    def __init__(self, kafka, topic, partition=None, version_0_7=True):
        self._kafka = kafka
        self._topic = topic
        self._partition = partition
        self._version_0_7 = version_0_7
